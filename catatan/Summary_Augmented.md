...
| `bitsandbytes` | Kuantisasi 4-bit | Menekan kebutuhan VRAM agar model 8B parameter bisa dimuat pada GPU dengan memori terbatas |
...
### Langkah 6 — Load Model Llama-3-8B (Base) dengan Kuantisasi 4-bit

**Pengecekan GPU:** notebook terlebih dulu memverifikasi ketersediaan GPU, nama GPU, kapasitas VRAM, dan dukungan BFloat16 — informasi ini penting karena proses loading dan inferensi model 8B parameter sangat bergantung pada spesifikasi hardware.
...
**Konfigurasi kuantisasi (`BitsAndBytesConfig`):**
```python
...
```
- `load_in_4bit=True` — memuat bobot model dalam presisi 4-bit, menghemat VRAM hingga sekitar 4x dibanding presisi penuh (FP32/FP16), sehingga model 8B parameter dapat dijalankan pada GPU dengan VRAM terbatas.
- `bnb_4bit_quant_type="nf4"` — menggunakan skema **NormalFloat4**, yang secara empiris memberikan akurasi lebih baik dibanding kuantisasi integer 4-bit biasa untuk bobot model yang terdistribusi normal.
- `bnb_4bit_compute_dtype=torch.bfloat16` — komputasi forward pass tetap dilakukan dalam BFloat16, format yang optimal untuk GPU A100.
- `bnb_4bit_use_double_quant=True` — menerapkan kuantisasi ganda untuk menghemat memori tambahan (~0.4 bit per parameter) tanpa mengorbankan akurasi secara signifikan.
...
**Loading model:**
- `device_map="auto"` — memanfaatkan `accelerate` untuk mendistribusikan layer model secara otomatis ke device yang tersedia (GPU/CPU).
- Model **tidak** dipanggil `.to()` atau `.eval()` secara manual karena model hasil kuantisasi 4-bit dari `bitsandbytes` sudah otomatis ditempatkan pada device yang benar, dan pemanggilan kedua method tersebut justru memicu `ValueError`.
...
- Notebook ini wajib dijalankan pada runtime dengan GPU memadai (direkomendasikan **A100** di Google Colab) mengingat ukuran model 8B parameter, meskipun sudah menggunakan kuantisasi 4-bit.
